...
const FeedbackCard = ({ index, testimonial, name, designation, image, link, profile }) => (
  <motion.div variants={fadeIn("", "spring", index * 0.5, 0.75)} className="bg-black-200 p-10 rounded-3xl xs:w-[320px] w-full">
    <p className="text-white font-black text-[48px]">"</p>
    <div className="mt-1">
      <a onClick={() => window.open(link, "_blank")} className="text-white tracking-wider text-[18px] cursor-pointer">{testimonial}</a>
      <div className="mt-7 flex justify-between items-center gap-1">
        <div className="flex-1 flex flex-col">
          <a onClick={() => window.open(profile, "_blank")} className="text-white font-medium text-[16px] cursor-pointer">
